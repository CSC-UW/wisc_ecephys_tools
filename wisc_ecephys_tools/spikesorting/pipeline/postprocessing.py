...
from ...sglx.experiments import get_ap_bin_paths
...
def run_postprocessing(
    ...
    modules = []
    params = []
    module_name_params = get_analysis_params("ks_postprocessing", analysis_name)
    ...
    if ks_dir.exists() and not rerun_existing:
        print(
            f"rerun_existing == False and kilosort dir exists at {ks_dir}...:\n Doing nothing.\n\n"
        )
        return
    elif ks_dir.exists() and rerun_existing:
        print(
            f"rerun_existing == True: Deleting post-processed kilosort dir at {ks_dir}.\n\n"
        )
        import shutil
