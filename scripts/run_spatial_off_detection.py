...
parser = argparse.ArgumentParser(
    description=(f"Run spatial off detection."),
    epilog=example_text,
...
parser.add_argument("--n_jobs", required=False, type=int, default=10, help="Acronym of atlas structure (eg 'Cx') restricting the returned structures")
...
def main():
    ...
    sorting = sglx_utils.load_singleprobe_sorting(
        sglxProject,
        sglxSubject,
        experiment,
        probe,
    ...
    out_dir = wet.get_sglx_project(
        outputProjectName
    ).get_experiment_subject_directory(
        experiment,
        subjectName,
    )
