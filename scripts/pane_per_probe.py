...
parser = argparse.ArgumentParser(
    description=(
        "Create one pane per completed subject/probe within "
        "an existing tmux session and type/run `<command_prefix>+'<subj>,<prb>'`"
        "in each pane.\n\n"
    ),
    epilog=example_text,
...
parser.add_argument(
    "--run", action="store_true", help="If present, we run the command in each pane."
)
parser.add_argument(
    "experiment", type=str, help="Name of experiment we search sortings for"
)
parser.add_argument("alias", type=str, help="Name of alias we search sortings for")
parser.add_argument(
    "command_prefix",
    type=str,
    help="Command to write in the pane. `'subj,prb'` is appended for each pane.",
)
...
subject_probes_list = get_subject_probe_list(
    args.experiment, args.alias, require_hypnogram_and_anatomy=True
)
...
for i, val in enumerate(subject_probes_list):
    ...
    subprocess.run("tmux split-window", shell=True)
    ...
    if args.run:
        ...
        subprocess.run(
            f"tmux send-keys -t {pane_i} '{prefix}{suffix}' Enter", shell=True
        )
    else:
